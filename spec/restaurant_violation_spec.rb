...
describe RestaurantViolation do 
  ...
  it "has a restauarant id" do
    expect(RestaurantViolation.find(rest_id: 3)).to include(@rv)
  end

  it "has a violation id" do
    expect(RestaurantViolation.find(vio_id: 2)).to include(@rv)
  end 

  it "matches restauarant and violations" do
    expect(RestaurantViolation.find(vio_id: 2).size).to eq(3)
  end
end